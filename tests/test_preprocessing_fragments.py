...
def test_read_dhs_to_memory(preprocessor):
    sites, length = preprocessor.read_dhs_to_memory()
    assert length == 1
    assert sites[0] == (5000, 'chr1')


def test_get_curr_dhs_empty(preprocessor):
    preprocessor.DHS_sites = []
    assert preprocessor.get_curr_dhs() == (None, None, None)


def test_get_curr_dhs(preprocessor):
    preprocessor.DHS_sites, _ = preprocessor.read_dhs_to_memory()
    start, end, chr_ = preprocessor.get_curr_dhs()
    assert (start, end, chr_) == (5000 - 1250, 5000 + 1250, 'chr1')


def test_parse_fragment(preprocessor):
    assert preprocessor.parse_fragment('chr1\t100\t250\t60\t+\n') == ('chr1', 100, 250)
...
def test_fragment_past_all_dhs(make_preprocessor):
    p = make_preprocessor(
        'chr1\t100\t200\n',
        ['chr1\t100000\t100150\t60\t+\n', 'chr1\t200000\t200150\t60\t+\n'],
    )
    assert np.sum(p.generate_matrix(should_save=False)) == 0
