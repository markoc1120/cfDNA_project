...
class FDIStatistic(TestStatistic):
    ...
    def calculate(self, matrix):
        ...
        dispersion_matrix = self.calculate_endpoint_dispersion(
            reads, matrix.shape[1], X, Y, ENDPOINT_WINDOW
        )

        # calculate FDI in non-overlapping windows
        non_overlap = self.calculate_windowed_fdi(
            coverage, dispersion_matrix, WINDOW_SIZE, MATRIX_SHIFT
        )
    ...
    def calculate_endpoint_dispersion(self, reads, matrix_columns, x, y, endpoint_window):
        ...
        for read in reads:
            ...
            local_density_start = (
                np.sum(dispersion_matrix[start - endpoint_window : start + endpoint_window + 1, 0])
                - 1
            )
            ...
            local_density_end = (
                np.sum(dispersion_matrix[end - endpoint_window : end + endpoint_window + 1, 0]) - 1
            )
    ...
    def calculate_sliding_windowed_fdi(
        self, coverage, dispersion_matrix, window_size, step_size, matrix_shift
    ):
        matrix_columns = len(coverage)
        if step_size <= 0:
            raise ValueError('step_size must be positive for overlapping windows')
        num_windows = (
            (matrix_columns - window_size) // step_size + 1 if matrix_columns >= window_size else 0
        )
    ...
    def visualize(self, statistic_data, output_paths):
        ...
        if 'non_overlapping_positions' in statistic_data:
            ...
            plt.plot(centers, statistic_data['non_overlapping_fdi_scores'], label='non-overlapping')
