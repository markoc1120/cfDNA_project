...
class OCFStatistic(TestStatistic):
    ...
    def calculate(self, matrix: np.ndarray):
        ...
        for pos in range(matrix_shift, matrix_columns - matrix_shift):
            ...
            starts_in_left_window = np.sum(
                fragment_starts[max(0, left_start) : min(matrix_columns, left_end)]
            )
            ends_in_left_window = np.sum(
                fragment_ends[max(0, left_start) : min(matrix_columns, left_end)]
            )

            starts_in_right_window = np.sum(
                fragment_starts[max(0, right_start) : min(matrix_columns, right_end)]
            )
            ends_in_right_window = np.sum(
                fragment_ends[max(0, right_start) : min(matrix_columns, right_end)]
            )
